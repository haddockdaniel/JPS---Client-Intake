            ...
            foreach (BillingField bb in bfList)
            {
                ...
                else
                {
                    if (bb.DBentryType.Equals("int"))
                        sql = "insert into DefaultSettings (DefaultID, [name], [data], entryType, empsys) values (999996, '" + bb.name.Replace(" ", "") + "', '0', '" + bb.DBentryType + "', " + empsysnbr.ToString() + ")";
                    else if (bb.DBentryType.Equals("date"))
                        sql = "insert into DefaultSettings (DefaultID, [name], [data], entryType, empsys) values (999996, '" + bb.name.Replace(" ", "") + "', '01/01/1900', '" + bb.DBentryType + "', " + empsysnbr.ToString() + ")";
                    else
