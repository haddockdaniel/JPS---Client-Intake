    ...
    public partial class CliBillingForm : Form
    {
        ...
        public bool loadFields()
        {
            ...
            if (dds2 != null && dds2.Tables.Count > 0)
            {
                ...
                if  (numOfFields == 0)
                {
                    MessageBox.Show("There are no defined Client Billing Fields in your data.", "Form Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                if (numOfFields < 11)
                {
        ...
        " values (999998, 'BFClient', 'N', getdate(), 'N', 'R')";
            ...
            foreach (BillingField bb in bfList)
            {
                ...
                {
                    sql = "insert into DefaultSettings (DefaultID, [name], [data], entryType) values (999998, '" + bb.name + "', '" + bb.text + "', 'richTextBox' )";
                    JU.ExecuteNonQuery(0, sql);
                }
            }
        }
